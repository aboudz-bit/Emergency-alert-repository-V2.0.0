...
- **Mapping**: `react-leaflet` with CartoDB Voyager light tiles for web. Mobile targets Google Maps (requires API key) with `react-native-maps`, falling back to Leaflet iframe for web preview. Features include zone polygons, location boundaries, and shelter management with real Khurais coordinates.
