...
- **Store**: `keas-mobile-store-v20` (persist key), version 24 — slice architecture (10 domain slices). Migration is wrapped in try/catch: on failure, resets to fresh seed data. v24 migration forces logout + backfills all missing arrays. ErrorFallback has "Clear Data & Restart" button to recover from corrupted AsyncStorage. User interface extended with `userType`, `companyType` (`Aramco`|`Contractor`), `companyName`, `mobileNumber`, `approvalStatus`, `approvedBy`, `approvedAt`, `rejectionReason`. UserRole extended with `'Supervisor' | 'Back Superior'`. Store includes `approveUser/rejectUser` actions, `ecoAssignments/supervisorAssignments` with merge protection, shelters slice with `addShelter/updateShelter/deleteShelter` actions. `useAlertSystemState` hook is the single source of truth for alert state across all screens. `selectAlertSystemState` unified selector with full null-safety.
