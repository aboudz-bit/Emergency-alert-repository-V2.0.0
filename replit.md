...
- **Alert Re-activation**: `selectIsEmergencyActive` checks alerts, zone alerts, AND emergency modes (shelterIn/blackout). `selectActiveAlert` synthesizes a virtual Alert (id=-1) from active zones OR emergency modes when no real alert exists — cache key includes emergency flags to prevent stale state. `deactivateZoneAlert`/`bulkDeactivateZoneAlerts` clear `personnelLocations` when no zones/alerts remain active. Full lifecycle supported: no-alert → active → clear → active again — repeatedly without app refresh.
...
- **Zone Management**: Admin "Zones and Locations" page (`/(admin)/locations`) with zone cards, active toggle, color dot, location count, "⋮" menu (Rename/Merge Into/Move Locations/Split Zone/Archive), multi-select mode with floating action bar, all 5 operation modals, archived zones section with restore. Store actions: `renameZone` (cascades to locations/users/ECO/supervisor assignments/emergency modes), `mergeZones` (moves locations/shelters/users to target, archives source), `moveLocationsBetweenZones`, `splitZone` (creates new zone with selected locations), `archiveZone` (blocks if alertActive). Zone type has optional `isArchived?: boolean`.
- **Zustand Selector Rule**: NEVER use `.filter()`, `.map()`, or object literals inside `useStore()` selectors — they create new references on every subscription callback, causing excessive re-renders and potential crashes in Zustand v5 + React 19. Instead: `const raw = useStore((s) => s.zones); const filtered = useMemo(() => raw.filter(...), [raw]);`
- **Store**: `keas-mobile-store-v20` (persist key), version 22 — slice architecture (10 domain slices). User interface extended with `userType`, `companyType` (`Aramco`|`Contractor`), `companyName`, `mobileNumber`, `approvalStatus`, `approvedBy`, `approvedAt`, `rejectionReason`. UserRole extended with `'Supervisor' | 'Back Superior'`. Store includes `approveUser/rejectUser` actions, `ecoAssignments/supervisorAssignments` with merge protection, shelters slice with `addShelter/updateShelter/deleteShelter` actions. Migration v21 backfills `companyType`/`companyName` on existing users.
- **Security hardening (audit fixes)**: Route guards on all 5 group layouts (admin, it, eco, supervisor, user) — unauthenticated users redirect to login, unauthorized role/assignment redirects to `/(user)`. Registration role locked to safe values only (User/Supervisor/Back Superior) — both UI and store enforcement. `closeAlert(-1)` routes to `sendAllClear` instead of being a no-op. Zone deletion blocked when zone or child locations have active alerts. `activateZoneAlert` resets `mobileUserResponse` and all user statuses to `pending`. `selectIsEmergencyActive` selector includes shelterIn/blackout emergency modes.
