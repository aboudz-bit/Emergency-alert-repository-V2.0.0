...
export default function MobileHome() {
  const { currentUser, respondToAlert, mobileUserResponse } = useStore(useShallow(s => ({
    ...
    <MobileLayout>
      ...
      <div className="px-6 py-5 bg-card border-b border-border flex justify-between items-center z-10 relative shadow-md">
        ...
        <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center border border-border">
          <span className="text-xs font-bold">{currentUser.name.split(' ').slice(0, 2).map(n => n[0]).join('')}</span>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 overflow-y-auto">
        ...
        <div className="bg-background border border-border rounded-2xl p-5 mb-6 flex flex-col items-center text-center shadow-lg relative overflow-hidden">
          ...
          <p className="text-sm text-muted-foreground font-mono mb-4">Badge: {currentUser.badge}</p>

          <div className="flex items-center gap-2 px-3 py-1.5 bg-card rounded-lg border border-border w-full justify-center">
            <MapPin className="w-4 h-4 text-primary" />
            <span className="text-sm font-semibold">{currentUser.zone} — {currentUser.location}</span>
          </div>
        </div>
        ...
        {activeAlert ? (
          <div className={cn(
            ...
            (hasResponded && isSafe)
              ...
              : (hasResponded && mobileUserResponse === 'need_help')
              ? 'bg-amber-500/20 border-2 border-amber-500'
              : 'bg-destructive/20 border-2 border-destructive animate-pulse shadow-destructive/20',
          )}>
            {hasResponded ? (
              <div className="space-y-4 animate-in zoom-in duration-300">
                <CheckCircle2 className={cn('w-20 h-20 mx-auto', isSafe ? 'text-safe' : 'text-amber-500')} />
                <h3 className={cn('text-2xl font-black', isSafe ? 'text-safe' : 'text-amber-500')}>
                  {isSafe ? 'RESPONSE RECORDED' : 'HELP REQUEST SENT'}
                </h3>
                <p className={cn('font-medium', isSafe ? 'text-safe/80' : 'text-amber-500/80')}>
                  {isSafe
                    ? 'You are marked as safe. Remain at your muster point until All Clear.'
                    : 'Emergency coordinator has been notified. Stay where you are.'}
                </p>
              </div>
            ) : (
